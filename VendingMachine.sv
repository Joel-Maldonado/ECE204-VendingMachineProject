module VendingMachine(
	 ...
	 logic [7:0] new_total;
	 assign new_total = balance + balance_addby;
	 ...
	 logic is_returning_change;
	 
	 always_ff @(posedge clk_1Hz or negedge reset_n) begin
		  if (!reset_n)
				is_returning_change <= 1'b0; // 0: Start in ACCEPTING mode
		  else
				is_returning_change <= is_returning_change; //todo: refund mode
	 end
	 ...
	 // vend when the new total reaches $1.00 (20 units)
	 assign vend = (!is_returning_change && balance_addby != 8'd0 && new_total >= 8'd20);

	 // clear the balance after vending
	 assign balance_clear_n = ~vend;


	 // outputs stay off until we add refund logic
	 assign nickel_out = 1'b0;
	 assign dime_out = 1'b0;
	 assign quarter_out = 1'b0;

	 
	 
	 
	 //todo: Add vend/ref logic to control is_returning_change
